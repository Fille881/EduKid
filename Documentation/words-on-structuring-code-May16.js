/*
Structuring code and the changes I made
...
and also managable. There is no single way that is 'the way' to do it, but a matter of opinion.
One more thing, it is not obligatory or anything... just a bit of a touch-up you can do if you feel like it.

pros: you can learn to write better code, and can make the current code more readable and managable
cons: you can mess it up for people, they have to learn the code again; also, you might spend time for no real benefit


--Below, I'll go through some javascript concepts that can aid in structuring a site.--

As an app gets bigger, it can be nice to separate things into modules. Preferably, functions
...
/*
What we see above is functions and variables defined inside another function. The outer function
then returns an object with references to the functions inside.
...
/*
Because there is no such member on the object we returned in the module -> it is accessible only within the module!
That is useful, because we don't need to worry about people using internal functions and variables
in weird ways in other parts of the app. It also means we are free to change anything we want inside our module,
without any other part of our app breaking because it depended on our 'internal' functions.

In addition, there is no risk for a name collision if some other part of the app creates a variable named 'items',
'items' here exists only within this function/module). We get one more benefit of naming the module also:
If you would just have a 'loose' function called addGrocery, that could mean
...
/*
For our EduKid site, I thought we had quite the number of functions. I thought I'd just move the map-related
functions to a separate file. However, that would not give the benefit of having a name like EduMap.changeBGcolor(),
so that is why I used the module pattern. The drawback is that it is some extra complexity for our group, and you have to
remember to add functions you want public to the 'return' in the bottom. Not sure it is worth it for our purposes.


Another thing I did was introduce a Player class that keeps every players points and countries
in one place (inside a Player object).
A first reason is that we started to have alot of player-related variables on our app-object:
*/
app.pointsP1 = 0;
app.pointsP2 = 0;
app.playerCounter = 0;
...
/*
Javascript does not have 'class' like  many other languages do
(where you can have functions inside the class etc). Instead you add
functions by setting them to the functions prototype-object.
Think of the functions as inside the Player() function. This may not be the most
elegant part of javascript, but well, it is how it is.
*/
...
// But what are some benefits with having this Player class instead
// of just loose variables? One thing is that we were able to easily create a function
// e.g. 'addcountry' for adding conquered countries
// to the player. It can be nice to keep functions closely tied to a player
...
/*
I wanted to see if I could make it simpler to get the currentPlayer-id, and also a
less error-prone way of switching the currentPlayer.
The thing with having a global playerCounter-variable is that every part of the app could change it,
which _might_ make things more tricky if several parts of the app touches that variable.
A solution would be to hide the real variable, so that you can't do whatever you like with it.
The only way to change the variable would be by to ask some function to do it. Like this:
*/
...
/*
This is like the module-pattern actually. We hide an variable, and only allow it to be accessed
through the 'change' or 'get' functions. The 'change' and 'get' are variables that each hold functions.
*/
...
/* It works, but I think I key-point is that how we always need to have this 'if'-statement
as soon as we need to do something with the current player.
But notice how the code is exactly the same in both if and esle, it is just the
playervariables we use that changes.
